...
class DriverDocumentCrud(CrudBase[DriverDocument, DriverDocumentSchema]):
    ...
    async def get_by_driver_profile_id(self, session: AsyncSession, driver_profile_id: int, **kwargs) -> list[DriverDocumentSchema]:
        ...
        documents = existing.scalars().all()
        ...
        if len(driver_profile.cars):
            ...
            for car_photo in car_photos:
                documents.append(DriverDocumentSchema(id=None, driver_profile_id=driver_profile_id, doc_type=f"CAR_PHOTO_{car_photo.type}", file_url=car_photo.photo_url, created_at=car_photo.created_at))

        return [self.schema.model_validate(document) for document in documents]
