...
class DriverDocumentCrud(CrudBase[DriverDocument, DriverDocumentSchema]):
    ...
    async def get_by_driver_profile_id(self, session: AsyncSession, driver_profile_id: int, **kwargs) -> list[DriverDocumentSchemaWithURL]:
        ...
        for item in items:
            ...
            documents.append(DriverDocumentSchemaWithURL(id=item.id, driver_profile_id=driver_profile_id, doc_type=item.doc_type, file_url=file_url, created_at=item.created_at, status=item.status, reviewed_at=item.reviewed_at, reviewed_by=item.reviewed_by))
        ...
        if len(driver_profile.cars):
            ...
            for car_photo in car_photos:
                documents.append(DriverDocumentSchemaWithURL(id=None, driver_profile_id=driver_profile_id, doc_type=f"CAR_PHOTO_{car_photo.type}", file_url=car_photo.photo_url, created_at=car_photo.created_at, status=car_photo.status))
