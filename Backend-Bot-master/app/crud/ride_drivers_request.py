...
class RideDriversRequestCrud(CrudBase[RideDriversRequest, RideDriversRequestSchema]):
    ...
    async def create(self, session: AsyncSession, create_obj: RideDriversRequestCreate) -> RideDriversRequestSchema | None:
        ...
        await fcm_service.send_to_user(session, ride.client_id, PushNotificationData(title="Новый отклик", body="На поездку откликнулся ещё один водитель"))
    ...
    async def _dispatch_accepted(self, session: AsyncSession, result: RideDriversRequestSchema, driver_profile: DriverProfileSchema, ride: RideSchema, **kwargs):
        accepted = await ride_crud.accept(session, result.ride_id, RideSchemaAcceptByDriver(driver_profile_id=result.driver_profile_id, offer_fare=result.offer_fare), driver_profile.user_id)
        if not accepted:
