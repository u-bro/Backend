...
class RideRouter(BaseRouter):
    ...
    async def accept_ride(self, request: Request, id: int, update_obj: RideSchemaAcceptByDriver, driver_profile_id: int = Depends(get_current_driver_profile_id), user_id: int = Depends(get_current_user_id)) -> RideSchema:
        ...
        if accepted is not None:
            ...
            await in_app_notification_crud.create(request.state.session, InAppNotificationCreate(user_id=accepted.client_id, type="ride_accepted", title="Your ride is accepted by driver", message="Wait for a driver, idk", dedup_key=str(ride.id)))
            return accepted
