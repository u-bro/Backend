...
class DriverDocumentRouter(BaseRouter):
    ...
    def setup_routes(self) -> None:
        ...
        self.router.add_api_route(f"{self.prefix}/{{id}}", self.get_by_id, methods=["GET"], status_code=200, dependencies=[Depends(require_role(["driver", "admin"])), Depends(require_driver_verification), Depends(require_driver_profile(DriverDocument))])
        self.router.add_api_route(f"{self.prefix}/{{id}}", self.update, methods=["PUT"], status_code=200, dependencies=[Depends(require_role(["driver", "admin"])), Depends(require_driver_verification), Depends(require_driver_profile(DriverDocument))])
