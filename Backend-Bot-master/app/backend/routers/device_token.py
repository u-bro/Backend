...
class DeviceTokenRouter(BaseRouter):
    ...
    async def get_my_tokens(self, request: Request, user: User = Depends(require_role([RoleCode.USER, RoleCode.DRIVER, RoleCode.ADMIN]))) -> List[DeviceTokenSchema]:
        items = await self.model_crud.get_by_user_id(request.state.session, user.id)
        return items

    async def get_my_token(self, request: Request, token: str, user: User = Depends(require_role([RoleCode.USER, RoleCode.DRIVER, RoleCode.ADMIN]))) -> DeviceTokenSchema:
        item = await self.model_crud.get_by_user_id_and_token(request.state.session, user.id, token)
        if not item:
            raise HTTPException(status_code=404, detail="Device token not found")
        return item

    async def create_my_token(self, request: Request, body: DeviceTokenIn, user: User = Depends(require_role([RoleCode.USER, RoleCode.DRIVER, RoleCode.ADMIN]))) -> DeviceTokenSchema:
    ...
    async def delete(self, request: Request, token: str, user: User = Depends(require_role([RoleCode.USER, RoleCode.DRIVER, RoleCode.ADMIN]))) -> DeviceTokenSchema | None:
