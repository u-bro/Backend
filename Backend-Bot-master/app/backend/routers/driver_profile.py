...
class DriverProfileRouter(BaseRouter):
    ...
    def setup_routes(self) -> None:
        ...
        self.router.add_api_route(f"{self.prefix}/{{id}}", self.update, methods=["PUT"], status_code=200, dependencies=[Depends(require_role(["driver", "admin"])), Depends(require_owner(DriverProfile, 'user_id'))])
        self.router.add_api_route(f"{self.prefix}/{{id}}", self.delete, methods=["DELETE"], status_code=202, dependencies=[Depends(require_role(["driver", "admin"])), Depends(require_owner(DriverProfile, 'user_id'))])
    ...
    async def update(self, request: Request, item_id: int, body: DriverProfileUpdate) -> DriverProfileSchema:
