...
class PhoneVerificationRouter(BaseRouter):
    ...
    def setup_routes(self) -> None:
        ...
        self.router.add_api_route(f"{self.prefix}/{{item_id}}", self.get_by_id, methods=["GET"], status_code=200, dependencies=[Depends(require_role("admin"))])
        self.router.add_api_route(self.prefix, self.create, methods=["POST"], status_code=201, dependencies=[Depends(require_role("admin"))])
        self.router.add_api_route(f"{self.prefix}/{{item_id}}", self.update, methods=["PUT"], status_code=200, dependencies=[Depends(require_role("admin"))])
        self.router.add_api_route(f"{self.prefix}/{{item_id}}", self.delete, methods=["DELETE"], status_code=200, dependencies=[Depends(require_role("admin"))])
