...
class DriverProfile(Base):
    ...
    birth_date = mapped_column(TIMESTAMP, nullable=True)
    photo_url: Mapped[str | None] = mapped_column(String, nullable=True)
    license_number: Mapped[str | None] = mapped_column(String(100), nullable=True)
    license_category: Mapped[str | None] = mapped_column(String(20), nullable=True)
    license_issued_at = mapped_column(TIMESTAMP, nullable=True)
    license_expires_at = mapped_column(TIMESTAMP, nullable=True)
